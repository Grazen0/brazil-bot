...
const daily = {
    ...
    execute: ({ channel, author, client }) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (diff < DAY_MILLIS) {
            ...
            channel.send(`You have to wait ${hours}h and ${minutes}m before claiming your reward!`);
            return;
        }
        const { dailyReward } = config_json_1.default;
        yield author.add(dailyReward);
        yield client.models.UserCurrency.upsert({
            user_id: author.id,
            last_daily: now,
        });
        channel.send(`You have received \`${utils_1.asCurrency(dailyReward)}\`!`);
    }),
};
